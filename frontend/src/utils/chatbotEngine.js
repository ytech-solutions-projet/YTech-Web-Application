...
export const getWelcomeReply = () => ({
  ...
  text: [
    ...
    '- PostgreSQL, securite et deploiement'
  ].join('\n'),
  actions: [
...
export const resolveChatbotReply = (rawMessage, history = []) => {
  ...
  const asksDatabase = includesOneOf(normalizedMessage, ['postgres', 'postgresql', 'base de donnees', 'database']);
  const asksTech = includesOneOf(normalizedMessage, ['react', 'node', 'backend', 'frontend', 'api', 'technique']);
  const asksDeploy = includesOneOf(normalizedMessage, ['deploy', 'deploiement', 'serveur', 'vmware', 'ubuntu', 'nginx', 'web server', 'db server']);
  const asksSecurity = includesOneOf(normalizedMessage, ['securite', 'vulnerabilite', 'faille', 'jwt', 'cookie', 'protection']);
  ...
  if ((asksGreeting && words.length <= 5) || asksWho) {
    return {
      ...
      text: [
        ...
        '- contact, messagerie, PostgreSQL et deploiement'
      ].join('\n'),
      actions: [
        createRouteAction('Voir les services', '/services'),
        createRouteAction('Demander un devis', '/devis'),
  ...
  if (asksDatabase || asksTech) {
    sections.push({
      title: 'Base technique',
      lines: [
        '- Le projet relie le frontend, le backend et PostgreSQL.',
        '- Les utilisateurs, devis, messages et demandes de contact sont relies a la base.',
        '- Le parcours principal tourne autour de React cote front et Node.js cote back.'
      ]
    });
  }
  ...
  if (asksDeploy) {
    sections.push({
      title: 'Deploiement',
      lines: [
        '- Le schema recommande est un web-server pour l application et un db-server pour PostgreSQL.',
        '- Nginx peut servir de proxy devant le backend.',
        '- Le web-server parle a PostgreSQL sur le reseau prive, pas en acces public.'
      ]
    });
  }
  ...
  if (asksSecurity) {
    sections.push({
      title: 'Securite',
      lines: [
        '- L application a deja un durcissement sur l auth, les roles, les endpoints et les validations.',
        '- Le but est de limiter l exposition des acces admin et des donnees sensibles.',
        '- Pour aller plus loin, il faut garder les secrets hors du chat et hors du front.'
      ]
    });
  }
  ...
  if (sections.length === 0) {
    ...
    return {
      topic: 'fallback',
      text: [
        'Je peux deja repondre sur presque tout le parcours YTECH : services, devis, prix, delais, paiement, dashboard, admin, contact, PostgreSQL et deploiement.',
        '',
